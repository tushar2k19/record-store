...
export default {
  ...
  created () {
    console.log("inside records -> created()")
    if (!localStorage.signedIn) {
      this.$router.replace('/')
    } else {
      this.$http.secured.get('/records')
        .then(response => { this.records = response.data })
        .catch(error => this.setError(error, 'Something went wrong'))

      this.$http.secured.get('/artists')
  ...
  methods: {
    ...
    addRecord () {
      ...
      this.$http.secured.post('/records', { record: { title: this.newRecord.title, year: this.newRecord.year, artist_id: this.newRecord.artist } })
    ...
    removeRecord (record) {
      this.$http.secured.delete(`/records/${record.id}`)
        .then(response => {
    ...
    updateRecord (record) {
      this.editedRecord = ''
      this.$http.secured.patch(`/records/${record.id}`, { record: { title: record.title, year: record.year, artist_id: record.artist } })
